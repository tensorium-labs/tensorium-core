...
print_summary() {
    ...
    echo -e "    ${CYAN}# Download txmminer-cuda from Releases${NC}"
    echo -e "    ${CYAN}# https://github.com/${REPO}/releases/latest${NC}"
    echo -e "    ${CYAN}chmod +x txmminer-cuda && sudo mv txmminer-cuda /usr/local/bin/${NC}"
    ...
    echo -e "    ${CYAN}txmminer-cuda pooltxm.tensoriumlabs.com:23336 ${miner_addr}${NC}"
    echo ""
