...
RELEASE_TAG="v0.4.0-mainnet" # technical asset tag; user-facing track is Mainnet v1
...
print_summary() {
    ...
    echo ""
    echo -e "  Start mining (GPU — NVIDIA 24 GB+ VRAM required, RTX 3090 minimum):"
    echo -e "    ${CYAN}# Build from source on a CUDA box:${NC}"
    echo -e "    ${CYAN}git clone https://github.com/${REPO}.git${NC}"
    echo -e "    ${CYAN}cd tensorium-core/tools/tensorium-miner && make${NC}"
    echo -e "    ${CYAN}./tensorium-miner --selftest${NC}"
    echo -e "    ${CYAN}# Pool mining (recommended, 5% fee):${NC}"
    echo -e "    ${CYAN}./tensorium-miner --mode pool --pool stratum+tcp://pooltxm.tensoriumlabs.com:3333 --wallet ${miner_addr} --worker rig1 --gpu all${NC}"
    echo -e "    ${CYAN}# Solo mining against your own node (0% fee):${NC}"
    echo -e "    ${CYAN}./tensorium-miner --mode solo --rpc http://127.0.0.1:${RPC_PORT} --wallet ${miner_addr} --gpu all${NC}"
    echo -e "    ${CYAN}# Solo mining against the public miner-compatible endpoint:${NC}"
    echo -e "    ${CYAN}./tensorium-miner --mode solo --rpc http://mc-rpc.tensoriumlabs.com --wallet ${miner_addr} --gpu all${NC}"
    echo -e "    ${CYAN}# Do not use https://rpc.tensoriumlabs.com for miner solo mode yet.${NC}"
    echo ""
